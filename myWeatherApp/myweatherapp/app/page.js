    ...
    try {
      ...
      if (response && response.list && response.list.length > 0) {
        ...
        const rainProcent = currentWeather.rain
          ...
          : 0;
        const humidityProcent = currentWeather.main.humidity;
        const windProcent = currentWeather.wind.speed;

        setRainProcent(Math.round(rainProcent));
        setHumidityProcent(Math.round(humidityProcent));
        setWindProcent(Math.round(windProcent));
      }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="lg:w-[43.75rem] lg:h-[54.38rem] md:w-[40rem] md:h-[53rem] sm:w-[35rem] sm:h-[30rem] flex flex-col items-center justify-center border-dashed border-4 border-white bg-white/10 backdrop-blur-lg rounded-[3vh]">
        ...
        <NextDays
          forecasts={forecasts}
          rainProcent={rainProcent}
          humidityProcent={humidityProcent}
          windProcent={windProcent}
        />
      </div>
    </div>
  );
